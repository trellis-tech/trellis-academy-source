...
function isRuntimeConfig(value: unknown): value is Record<string, string> {
  return value !== null
    && typeof value === 'object'
    && !Array.isArray(value)
...
function loadRuntimeConfig(): Record<string, string> {
  ...
  return isRuntimeConfig(window.__RUNTIME_CONFIG__)
...
export const getConfig = (key: string, defaultValue: string = ''): string => {
  ...
  // 1. Check browser runtime config injected by the standalone server.
