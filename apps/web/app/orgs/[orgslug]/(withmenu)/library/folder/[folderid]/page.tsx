...
const FolderPage = async ({ params }: { params: Promise<{ orgslug: string; folderid: string }> }) => {
  const { orgslug, folderid } = await params
  const session = await getServerSession()
  if (session?.unresolved) {
    const destination = `/library/folder/${encodeURIComponent(folderid)}`
    redirect(`/api/auth/refresh?destination=${encodeURIComponent(destination)}`)
  }
  ...
  const folder = await fetchFolderForViewer(folderid)
  if (!folder) {
    notFound()
  }
