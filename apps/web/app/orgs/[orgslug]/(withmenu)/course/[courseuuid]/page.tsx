...
export async function generateMetadata(props: MetadataProps): Promise<Metadata> {
  ...
  if (!courseResult) {
    return {
      title: `Course — ${org?.name || 'Trellis Academy'}`,
      description: 'View this course in Trellis Academy',
    }
  }
  ...
  const canonical = await getServerCanonicalUrl(params.orgslug, `/course/${params.courseuuid}`)
...
const CoursePage = async ({ params }: { params: Promise<{ courseuuid: string; orgslug: string }> }) => {
  const { courseuuid, orgslug } = await params
  return <CourseClient courseuuid={courseuuid} orgslug={orgslug} />
}
