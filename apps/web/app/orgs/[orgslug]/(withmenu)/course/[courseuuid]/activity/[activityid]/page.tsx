...
export async function generateMetadata(props: MetadataProps): Promise<Metadata> {
  ...
  const rawTitle = isCourseEnd ? `Congratulations — ${course_meta.name} Course` : `${activity?.name ?? 'Lesson'} — ${course_meta.name} Course`
  ...
  const imageUrl = course_meta?.thumbnail_image
    ...
    : orgOgImageUrl || new URL('/trellis-academy.svg', canonical).toString()
...
const ActivityPage = async ({ params }: { params: Promise<{ activityid: string; courseuuid: string; orgslug: string }> }) => {
  const { activityid, courseuuid, orgslug } = await params
  return <ActivityClient activityid={activityid} courseuuid={courseuuid} orgslug={orgslug} />
}
