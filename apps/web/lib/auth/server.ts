...
export interface AcademyServerSession {
  user: {
    id: number
    email: string
    first_name: string
...
export async function hasAcademyRefreshSession(): Promise<boolean> {
  const cookieStore = await cookies()
  return Boolean(cookieStore.get(REFRESH_TOKEN_COOKIE)?.value)
}

export async function getAcademyServerSession(): Promise<AcademyServerSession | null> {
  const accessToken = await getServerAccessToken()
  if (!accessToken) return null

  try {
    const response = await fetch(`${BACKEND_URL}/api/v1/auth/trellis/session`, {
      method: 'GET',
    ...
    const academyRole = Reflect.get(value, 'academy_role')
    if (!user || typeof user !== 'object') return null
    const id = Reflect.get(user, 'id')
    const email = Reflect.get(user, 'email')
    const firstName = Reflect.get(user, 'first_name')
    const lastName = Reflect.get(user, 'last_name')
    if (
      typeof id !== 'number'
      || typeof email !== 'string'
      || typeof firstName !== 'string'
      || typeof lastName !== 'string'
      || (academyRole !== 'owner' && academyRole !== 'learner')
    ) return null
    return {
      user: { id, email, first_name: firstName, last_name: lastName },
      academy_role: academyRole,
    }
  } catch (error) {
    console.error('[ACADEMY_SESSION] Error:', error)
    return null
  }
}
