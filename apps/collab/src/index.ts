...
const pendingFlushes = new Map<string, { timer: NodeJS.Timeout; state: Uint8Array }>()
const dbFlushChains = new Map<string, Promise<void>>()

async function persistYdoc(boardUuid: string, state: Uint8Array) {
  ...
  if (!response.ok) {
    throw new Error(`DB flush failed for ${boardUuid}: ${response.status}`)
  }
}

function enqueueDbFlush(boardUuid: string, state: Uint8Array) {
  const previous = dbFlushChains.get(boardUuid) ?? Promise.resolve()
  const next = previous
    .catch((err) => {
      console.error(`[collab] Previous DB flush error for ${boardUuid}:`, err)
    })
    .then(() => persistYdoc(boardUuid, state))

  dbFlushChains.set(boardUuid, next)
  void next
    .catch((err) => {
      console.error(`[collab] DB flush error for ${boardUuid}:`, err)
    })
    .finally(() => {
      if (dbFlushChains.get(boardUuid) === next) {
        dbFlushChains.delete(boardUuid)
      }
    })
}
...
function scheduleDbFlush(boardUuid: string, state: Uint8Array) {
  ...
  const timer = setTimeout(async () => {
    ...
    enqueueDbFlush(boardUuid, stateCopy)
  }, DB_FLUSH_DELAY)
      ...
      async fetch({ documentName }) {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(`DB fetch failed for ${boardUuid}: ${response.status}`)
          }
...
async function gracefulShutdown() {
  ...
  for (const [boardUuid, { timer, state }] of pendingFlushes) {
    clearTimeout(timer)
    enqueueDbFlush(boardUuid, state)
  }

  await Promise.allSettled([...dbFlushChains.values()])
