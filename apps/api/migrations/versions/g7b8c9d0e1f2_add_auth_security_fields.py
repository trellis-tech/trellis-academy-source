...
def upgrade() -> None:
    ...
    # Preserve unverified accounts. Only backfill the timestamp for users whose
    # existing verified flag already proves they completed verification.
    op.execute("""
        UPDATE "user"
        SET email_verified_at = NOW()::text
