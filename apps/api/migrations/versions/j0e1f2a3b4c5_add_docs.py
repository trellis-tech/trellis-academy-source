...
def upgrade() -> None:
    ...
    # Preserve the privilege level of organization-specific roles. Documentation
    # permissions mirror each role's existing course permissions; roles without
    # course rights receive the read-only user default.
    user_perms_json = json.dumps(DOCSPACES_PERMISSIONS[4])
    conn.execute(
        sa.text(
            """
            UPDATE role
            SET rights = jsonb_set(
                CAST(rights AS jsonb),
                '{docspaces}',
                COALESCE(CAST(rights AS jsonb) -> 'courses', CAST(:perms AS jsonb))
            )
            WHERE id NOT IN (1, 2, 3, 4)
              AND rights IS NOT NULL
              AND CAST(rights AS text) != ''
            """
        ),
        {"perms": user_perms_json},
    )
