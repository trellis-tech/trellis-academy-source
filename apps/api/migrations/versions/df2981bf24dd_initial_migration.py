"""Legacy first delta migration.

This revision predates a complete Alembic baseline and expects the historical
LearnHouse schema. Fresh Academy databases are created from current SQLModel
metadata and stamped at the matching head by ``cli.py install`` through
``src.core.schema_bootstrap.create_schema_and_stamp``. Do not run this legacy
delta directly against an empty database.
