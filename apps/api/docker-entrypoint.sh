...
if [ -n "$LEARNHOUSE_REDIS_CONNECTION_STRING" ]; then
    REDIS_TARGET=$(cd "$SCRIPT_DIR" && "$PYTHON_BIN" -m src.core.connection_url \
        "$LEARNHOUSE_REDIS_CONNECTION_STRING" 6379)
    read -r REDIS_HOST REDIS_PORT <<< "$REDIS_TARGET"
...
echo "Starting Trellis Academy backend on ${HOST}:${PORT}..."
