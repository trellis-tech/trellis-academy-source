...
ACADEMY_EXCLUDED_MODEL_MODULES = frozenset({"src.db.course_embeddings"})
    ...
    for config in model_configs:
        ...
        for root, dirs, files in os.walk(base_dir):
            ...
            for file_name in module_files:
                ...
                if (
                    os.getenv("TRELLIS_ACADEMY_RUNTIME", "").lower()
                    in ("1", "true", "yes")
                    and full_module_path in ACADEMY_EXCLUDED_MODEL_MODULES
                ):
                    continue
                try:
...
async def _bootstrap_schema():
    # Extension creation must not share the schema transaction. PostgreSQL
    # aborts the entire transaction when CREATE EXTENSION fails, even if Python
    # catches the exception, which would poison the following create_all call.
    if not is_testing and engine.dialect.name == "postgresql":
        try:
...
async def connect_to_db(app: FastAPI, *, bootstrap_schema: bool = True):
