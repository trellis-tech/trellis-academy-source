...
from src.academy.learning_policy import (
...
async def _lock_assignment_attempt(
    db_session: AsyncSession, assignment_id: int, user_id: int
) -> None:
    """Serialize one learner's answer edits and hand-in transaction."""
    if db_session.get_bind().dialect.name != "postgresql":
        return
    await db_session.execute(
        text("SELECT pg_advisory_xact_lock(:assignment_id, :user_id)"),
...
async def read_assignment_from_activity_uuid(
    ...
    statement = (
        ...
        .order_by(Assignment.published.desc(), Assignment.id.desc())
    )
    row = (await db_session.execute(statement)).first()
...
async def update_assignment_task_submission(
    ...
    if is_instructor:
        ...
        await _lock_assignment_attempt(db_session, assignment.id, current_user.id)
        if _is_assignment_past_due(assignment):
        ...
        if parent_submission is not None and parent_submission.submission_status not in (
            AssignmentUserSubmissionStatus.PENDING,
            AssignmentUserSubmissionStatus.NOT_SUBMITTED,
        ):
            raise HTTPException(
                status_code=403,
                detail="This assignment has already been handed in. Use retry to attempt it again.",
            )
        assignment_task_submission_object = assignment_task_submission_object.model_copy(
            update={
                "grade": None,
                "task_submission_grade_feedback": None,
                "assignment_task_id": None,
                "assignment_type": None,
                "manually_graded": None,
            }
        )
...
async def create_assignment_submission(
    ...
    await _lock_assignment_attempt(db_session, assignment.id, submitter.id)
