...
async def check_domain_ssl_status(
    ...
    try:
        validated_ips = resolve_and_validate_url(f"https://{domain.domain}")
    ...
    try:
        ...
        # Connect to the address that passed validation. The original hostname
        # is retained only for TLS SNI and certificate verification, closing the
        # DNS-rebinding window between validation and connect.
        validated_ip = sorted(validated_ips)[0]
        with socket.create_connection((validated_ip, 443), timeout=5) as sock:
