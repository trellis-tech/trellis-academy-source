...
async def query_course_rag(
    ...
    Args:
        ...
        course_id: Optional course ID to scope to a single course.
        allowed_course_ids: Read-authorized course IDs for an org-wide query.
    ...
    """
    if course_id is None and not allowed_course_ids:
    ...
    if course_id is not None:
        ...
        sql = text("""
            ...
            FROM course_embedding ce
            JOIN course c ON c.id = ce.course_id
            WHERE ce.org_id = :org_id AND ce.course_id IN :allowed_course_ids
            ORDER BY ce.embedding <=> :query_embedding
            LIMIT :top_k
        """).bindparams(bindparam("allowed_course_ids", expanding=True))
        params = {
            "query_embedding": embedding_str,
            "org_id": org_id,
