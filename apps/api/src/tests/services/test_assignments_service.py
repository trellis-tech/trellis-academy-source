...
_PATCH_ATTEMPT_LOCK = (
    "src.services.courses.activities.assignments._lock_assignment_attempt"
)
_PATCH_CERT = (
...
class TestReadAssignmentFromActivityUuid:
    ...
    async def test_prefers_the_published_assignment_version(
        self, mock_request, db, assignment, activity, admin_user
    ):
        assignment.published = False
        current = Assignment(
            **assignment.model_dump(exclude={"id", "assignment_uuid", "published"}),
            assignment_uuid="assignment_current_version",
            published=True,
        )
        db.add(assignment)
        db.add(current)
        ...
        assert result.assignment_uuid == current.assignment_uuid
...
class TestUpdateAssignmentTaskSubmission:
    ...
    async def test_non_instructor_grade_and_feedback_stripped(
        ...
        with patch(_PATCH_RBAC, new_callable=AsyncMock), \
             patch(_PATCH_AUTH_ROLES, new_callable=AsyncMock, return_value=False), \
             patch(_PATCH_ATTEMPT_LOCK, new_callable=AsyncMock) as attempt_lock:
            result = await update_assignment_task_submission(
                mock_request,
                task_submission.assignment_task_submission_uuid,
                payload,
        ...
        attempt_lock.assert_awaited_once_with(db, assignment_task.assignment_id, regular_user.id)
    ...
    async def test_non_instructor_cannot_update_answer_after_hand_in(
        self,
        mock_request,
        db,
        regular_user,
        ...
        parent = AssignmentUserSubmission(
            assignment_id=assignment.id,
            user_id=regular_user.id,
        ...
        db.add(parent)
        await db.commit()
        payload = AssignmentTaskSubmissionUpdate(task_submission={"answer": "tampered"})
...
class TestCreateAssignmentSubmission:
    async def test_creates_trailrun_and_trailstep_when_missing(
        ...
        attempt_lock.assert_awaited_once_with(db, assignment.id, admin_user.id)
