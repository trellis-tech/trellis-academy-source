...
class TestRBACRuntime:
    ...
    @pytest.mark.asyncio
    async def test_check_usergroup_access_allows_open_resource_for_org_member(self):
        session = _session_with_results(
            _result(all=[]),
            _result(first=SimpleNamespace(user_id=99, org_id=7)),
        )
    ...
    @pytest.mark.asyncio
    async def test_check_usergroup_access_denies_unresolved_open_resource(self):
    ...
    @pytest.mark.asyncio
    async def test_paid_enrollment_grants_before_usergroup_paywall(self):
        payments_module = ModuleType("ee.services.payments.payments_access")
        enrollment_check = AsyncMock(return_value=True)
        payments_module.check_enrollment_access = enrollment_check
        session = AsyncMock()

        with patch.dict(
            sys.modules,
            {"ee.services.payments.payments_access": payments_module},
        ), patch(
            ...
            new=AsyncMock(side_effect=HTTPException(status_code=402)),
        ) as usergroup_check:
            allowed = await authorization_verify_based_on_roles_and_authorship(
                request=_request(),
                user_id=7,
                action="read",
        ...
        enrollment_check.assert_awaited_once()
        usergroup_check.assert_not_awaited()
