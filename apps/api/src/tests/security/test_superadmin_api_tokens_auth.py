...
from src.security.auth import get_current_user, resolve_acting_user_id, validate_superadmin_api_token
from src.security.superadmin import require_superadmin

reject_any_api_token = ee_superadmin_tokens.reject_any_api_token


def test_superadmin_api_token_resolves_to_minting_user():
    principal = SuperadminAPITokenUser(
        id=999,
        user_uuid="satoken_999",
        token_name="test",
        created_by_user_id=42,
    )
    assert resolve_acting_user_id(principal) == 42
