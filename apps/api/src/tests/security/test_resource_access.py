...
class TestResourceConfig:
    ...
    def test_get_resource_config_for_activity(self):
        """Test getting config for activity UUID (child of course)."""
...
class TestParentResourceResolution:
    ...
    def test_activity_has_course_parent(self):
        """Test that activity config follows the Activity.course_id relation."""
...
class TestDashboardContext:
    ...
    def _make_checker(self, request, session, user, **overrides):
        ...
        checker._has_linked_usergroups = AsyncMock(
            return_value=overrides.get("linked_usergroup", False)
        )
        return checker
