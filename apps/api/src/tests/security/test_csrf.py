...
class TestCSRFExemptions:
    ...
    def test_resend_svix_webhook_exempt_only_on_delivery_route(self):
        mw = self._make_middleware()
        headers = {
            "svix-id": "msg_1",
            "svix-timestamp": "123",
        ...
        req.url.path = "/api/v1/internal/emails/delivery-events"
        assert mw._is_csrf_exempt(req) is True

        req.url.path = "/api/v1/courses"
        assert mw._is_csrf_exempt(req) is False

    def test_resend_svix_webhook_requires_all_signature_headers(self):
        mw = self._make_middleware()
        req = _make_request(headers={"svix-id": "msg_1", "svix-timestamp": "123"})
        req.url.path = "/api/v1/internal/emails/delivery-events"
        assert mw._is_csrf_exempt(req) is False
