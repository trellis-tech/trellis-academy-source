...
class TestAuth:
    ...
    @pytest.mark.asyncio
    async def test_trellis_session_fails_closed_when_subject_is_inactive(
        ...
        with (
            patch(
                "src.services.auth.trellis_sso.introspect_trellis_subject",
                new_callable=AsyncMock,
                create=True,
                return_value=False,
            ) as introspect,
            patch(
                "src.security.auth.security_get_user",
                new_callable=AsyncMock,
                return_value=mock_user,
            ),
            patch("src.security.auth.revoke_user_sessions_before") as revoke,
            pytest.raises(HTTPException) as denied,
        ):
            await get_current_user(request=mock_request, db_session=mock_db_session)

        assert denied.value.status_code == 401
        introspect.assert_awaited_once_with(subject)
    ...
    async def test_get_current_user_stale_token_naive_password_changed_at_raises_401(
        ...
        mock_user.password_changed_at = datetime.now(timezone.utc).replace(tzinfo=None)
