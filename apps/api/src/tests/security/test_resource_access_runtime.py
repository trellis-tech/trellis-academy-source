...
class TestResourceAccessRuntime:
    ...
    @pytest.mark.asyncio
    async def test_api_token_child_permissions_use_child_bucket_and_parent_org(
        self, mock_request, session
    ):
        token = APITokenUser(
            org_id=11,
            rights={"activities": {"action_update": True}},
        )
        checker = self._checker(mock_request, session, token)
        checker._get_resource = AsyncMock(return_value=SimpleNamespace(org_id=11))

        allowed = await checker.check_access("activity_1", AccessAction.UPDATE)
        ...
        checker = self._checker(
            mock_request,
            session,
            APITokenUser(org_id=11, rights={"courses": {"action_update": True}}),
        )
        checker._get_resource = AsyncMock(return_value=SimpleNamespace(org_id=11))

        denied = await checker.check_access("activity_1", AccessAction.UPDATE)
