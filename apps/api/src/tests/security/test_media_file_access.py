...
async def _mk_folder(db, org, public, name, parent_folder_id=None):
    f = Folder(
...
class TestMediaFileAccess:
    ...
    @pytest.mark.asyncio
    async def test_public_media_below_private_ancestor_denied_for_anon(
        self, db, org, anonymous_user, mock_request
    ):
        m = await _mk_media(db, org, public=True, name="nestedpriv")
        private_parent = await _mk_folder(db, org, public=False, name="private-parent")
        public_child = await _mk_folder(
            db,
            org,
            public=True,
            name="public-child",
        ...
        await _place(db, org, public_child.id, m.media_uuid)
...
class TestCheckContentAccessMediaBranch:
    ...
    async def test_legacy_media_path_checks_access(self, db, org, admin_user, mock_request, module):
        ...
        with patch.object(mod, "check_resource_access", new_callable=AsyncMock) as chk:
