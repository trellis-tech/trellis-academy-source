...
class TestCheckContentAccess:
    ...
    def _make_db_session(self, course=None, podcast=None, activity=None):
        session = MagicMock()
        if activity is not None:
            activity_result = MagicMock()
            activity_result.scalars.return_value.first.return_value = activity
            course_result = MagicMock()
            course_result.scalars.return_value.first.return_value = course
            session.execute = AsyncMock(side_effect=[activity_result, course_result])
            return session
    ...
    async def test_public_course_activity_anonymous(self):
        ...
        activity = SimpleNamespace(course_id=course.id)
        db = self._make_db_session(course=course, activity=activity)
        # Should not raise
        with patch(
            "src.routers.local_content.check_resource_access",
            new=AsyncMock(return_value=SimpleNamespace(allowed=True, reason="public")),
        ):
    ...
    async def test_private_course_activity_anonymous_rejected(self):
        ...
        with patch(
            "src.routers.local_content.check_resource_access",
            new=AsyncMock(return_value=SimpleNamespace(allowed=False, reason="private")),
    ...
    async def test_private_course_activity_authenticated(self):
        ...
        with patch(
            "src.routers.local_content.check_resource_access",
            new=AsyncMock(return_value=SimpleNamespace(allowed=True, reason="member")),
        ):
            await _check_content_access(
                "orgs/org1/courses/course_abc/activities/act1/video.mp4",
    ...
    @pytest.mark.asyncio
    async def test_unknown_path_auth_rejected(self):
...
class TestS3ContentAccess:
    ...
    async def test_public_course_anonymous_allowed(self):
        ...
        db = self._make_db_session(course=course, activity=activity)
        with patch(
            "src.routers.content_files.check_resource_access",
            new=AsyncMock(return_value=SimpleNamespace(allowed=True, reason="public")),
        ):
            await _check_content_access(
    ...
    @pytest.mark.asyncio
    async def test_unknown_path_anon_rejected(self):
        from src.routers.content_files import _check_content_access
        db = self._make_db_session()
        with pytest.raises(HTTPException) as exc_info:
            await _check_content_access(
