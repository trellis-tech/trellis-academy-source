...
async def test_activity_grounding_requires_course_update_access():
    body = GenerateScenarioRequest(org_id=5, prompt="p", activity_uuid="activity_1")
    activity = SimpleNamespace(id=3, course_id=4, content={"blocks": []})
    course = SimpleNamespace(id=4, org_id=5, course_uuid="course_1")
    request = MagicMock()

    with patch.object(sc, "is_org_member", new=AsyncMock(return_value=True)), \
         patch.object(sc, "enforce_org_mfa", new=AsyncMock()), \
         patch.object(sc, "check_resource_access", new=AsyncMock(side_effect=HTTPException(status_code=403))) as access:
        with pytest.raises(HTTPException) as exc:
            await sc.api_generate_scenario(
                body,
                _user(),
                _db_returning(_org(), activity, course),
                request,
            )
