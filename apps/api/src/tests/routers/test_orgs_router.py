...
class TestOrgCrudEndpoints:
    ...
    async def test_create_org_with_v2_config_forces_free_plan(self, client):
        create = AsyncMock(return_value=_mock_org_read())
        with patch("src.routers.orgs.orgs.create_org_with_config", create):
            response = await client.post(
                "/api/v1/orgs/withconfig/",
                json={
                    "org_object": {
                    ...
                    "config_object": {"config_version": "2.0", "plan": "enterprise"},
                },
            )

        assert response.status_code == 200
        submitted = create.await_args.args[-1]
        assert submitted.plan == "free"
