...
async def test_activity_update_denial_happens_before_credit_spend():
    denial = HTTPException(status_code=403, detail="no update access")
    with patch.object(aud, "check_resource_access", new=AsyncMock(side_effect=denial)), \
         patch.object(aud, "is_org_member", new=AsyncMock(return_value=True)), \
         patch.object(aud, "reserve_ai_credit", new=AsyncMock()) as reserve:
        with pytest.raises(HTTPException) as exc:
            await aud.api_generate_audio(
                MagicMock(), _body(), _user(), _db_returning(_activity())
            )
...
async def test_store_error_refunds_and_fails():
    with patch.object(aud, "is_org_member", new=AsyncMock(return_value=True)), \
         patch.object(aud, "enforce_ai_rate_limit"), \
         patch.object(aud, "reserve_ai_credit", new=AsyncMock()), \
         patch.object(aud, "generate_speech", new=AsyncMock(return_value=b"RIFFxxxxWAVE")), \
...
async def test_script_activity_update_denial_happens_before_credit_spend():
    denial = HTTPException(status_code=403, detail="no update access")
    with patch.object(aud, "check_resource_access", new=AsyncMock(side_effect=denial)), \
         patch.object(aud, "is_org_member", new=AsyncMock(return_value=True)), \
         patch.object(aud, "reserve_ai_credit", new=AsyncMock()) as reserve:
        with pytest.raises(HTTPException) as exc:
            await aud.api_generate_script(
                MagicMock(), _script_body(), _user(), _db_returning(_activity())
            )
    assert exc.value.status_code == 403
    reserve.assert_not_called()
...
async def test_script_happy_path_speak_monologue():
    with patch.object(aud, "is_org_member", new=AsyncMock(return_value=True)), \
        ...
        resp = await aud.api_generate_script(
            MagicMock(), _script_body(mode="speak", minutes=3), _user(), _db_returning(_activity())
        )
