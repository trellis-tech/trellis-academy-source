...
class TestGenerateActivityContentIsTenantScoped:
    ...
    async def test_session_in_org_a_cannot_reach_activity_in_org_b(
        ...
        session = _session(org.id, user_id=admin_user.id)
    ...
    async def test_plain_member_cannot_generate_into_someone_elses_course(
        ...
        session = _session(org.id, user_id=regular_user.id)
        stream = _stream_spy(session)
        reserve = AsyncMock()
...
class TestCoursePlanningSessionOwnership:
    async def test_session_state_is_hidden_from_another_org_member(
        ...
        with patch.object(cp, "get_course_planning_session", return_value=session):
            with pytest.raises(HTTPException) as exc:
                await cp.get_session_state(session.session_uuid, regular_user, db)
