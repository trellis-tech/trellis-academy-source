...
class TestContentFilesRouter:
    ...
    async def test_check_content_access_course_and_podcast_branches(
        ...
        private_activity = Activity(
            ...
            activity_uuid="activity_private_access",
        )
        public_activity = Activity(
            ...
            activity_uuid="activity_public_access",
        )
        ...
        await content_files._check_content_access(
            f"orgs/{org.org_uuid}/courses/{public_course.course_uuid}/activities/{public_activity.activity_uuid}/video.mp4",
            AnonymousUser(),
            db,
        )
        ...
        with pytest.raises(Exception) as anon_private_course:
            await content_files._check_content_access(
                f"orgs/{org.org_uuid}/courses/{private_course.course_uuid}/activities/{private_activity.activity_uuid}/video.mp4",
                AnonymousUser(),
                db,
            )
        ...
        await content_files._check_content_access(
            f"orgs/{org.org_uuid}/courses/{private_course.course_uuid}/activities/{private_activity.activity_uuid}/video.mp4",
            APITokenUser(org_id=org.id, rights={"activities": {"action_read": True}}),
            db,
        )
