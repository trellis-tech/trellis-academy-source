...
def test_academy_model_discovery_does_not_require_vector_models(monkeypatch):
    monkeypatch.setenv("TRELLIS_ACADEMY_RUNTIME", "1")
    monkeypatch.setattr(database.os.path, "exists", lambda path: path == "src/db")
    monkeypatch.setattr(
        database.os,
        "walk",
        lambda _base_dir: iter(
            [("src/db", [], ["courses.py", "course_embeddings.py", "__init__.py"])]
        ),
    )
    imported: list[str] = []
    monkeypatch.setattr(database.importlib, "import_module", imported.append)

    database.import_all_models()

    assert imported == ["src.db.courses"]
