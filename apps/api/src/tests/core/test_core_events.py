...
async def test_check_ee_activity_paid_access_defaults_to_free(monkeypatch):
    ...
    monkeypatch.setattr("src.core.deployment_mode.get_deployment_mode", lambda: "oss")
...
@pytest.mark.asyncio
async def test_check_ee_activity_paid_access_fails_closed_in_saas(monkeypatch):
    monkeypatch.setattr(ee_hooks, "get_ee_hooks", lambda: None)
