...
ZAPIER_RIGHTS = {
    ...
    "usergroups": {"action_read": True},
    "organizations": {"action_read": True, "action_update": True},
}
...
class TestRequireApiToken:
    ...
    async def test_list_rejects_token_without_course_read_right(self, db, org, user):
        token = APITokenUser(
            id=88,
            org_id=org.id,
            created_by_user_id=user.id,
        ...
        with pytest.raises(HTTPException) as exc:
            await zapier_list_courses(ctx=(token, db))
