...
class AssignmentTaskSubmissionCreate(SQLModel):
    """Learner-editable fields for creating a task submission.

    Identity, task type, grade, feedback, and manual-grading state are derived
    by the server from the authenticated learner and assignment task.
    """

    task_submission: Dict = Field(default_factory=dict)
