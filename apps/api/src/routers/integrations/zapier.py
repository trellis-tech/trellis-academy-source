...
def _require_token_permission(
    api_user: APITokenUser,
    resource: str,
    action: str,
) -> None:
    """Enforce the API token's configured permission bucket."""
    rights = api_user.rights
    if isinstance(rights, dict):
        resource_rights = rights.get(resource)
    else:
        resource_rights = getattr(rights, resource, None) if rights else None

    key = f"action_{action}"
    if isinstance(resource_rights, dict):
        allowed = bool(resource_rights.get(key, False))
    else:
        allowed = bool(getattr(resource_rights, key, False))
    if not allowed:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"API token lacks {action} permission for {resource}",
        )
...
async def zapier_list_courses(
    ...
    api_user, db_session = ctx
    _require_token_permission(api_user, "courses", "read")
    query = (
        select(Course)
...
async def zapier_list_users(
    ...
    api_user, db_session = ctx
    _require_token_permission(api_user, "users", "read")
    query = (
        select(User)
...
async def zapier_list_usergroups(
    ...
    api_user, db_session = ctx
    _require_token_permission(api_user, "usergroups", "read")
    query = (
        select(UserGroup)
...
async def zapier_create_subscription(
    ...
    api_user, db_session = ctx
    _require_token_permission(api_user, "organizations", "update")
...
async def zapier_list_subscriptions(
    ...
    api_user, db_session = ctx
    _require_token_permission(api_user, "organizations", "read")
