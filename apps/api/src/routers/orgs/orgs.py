...
from src.db.organization_config import OrganizationConfigBase, OrganizationConfigV2Base
...
async def api_create_org_withconfig(
    request: Request,
    org_object: OrganizationCreate,
    config_object: OrganizationConfigBase | OrganizationConfigV2Base,
    current_user: PublicUser = Depends(get_current_user),
    db_session: AsyncSession = Depends(get_db_session),
) -> OrganizationRead:
    """
    ...
    if isinstance(config_object, OrganizationConfigV2Base):
        config_object.plan = "free"
    elif config_object.cloud is not None:
