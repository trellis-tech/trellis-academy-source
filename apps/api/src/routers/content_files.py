...
async def _check_content_access(
    ...
    # Activity content follows the activity's full RBAC chain, including
    # chapter/activity UserGroup locks and API-token permissions.
    if (
        len(parts) >= 6
        and parts[0] == 'orgs'
        and parts[2] == 'courses'
        ...
        if not activity or not course or activity.course_id != course.id:
