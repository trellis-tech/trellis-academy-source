...
@router.post(
    ...
    description="Streaming RAG chatbot grounded in course content. If `course_uuid` is provided, searches within that authorized course only. Otherwise searches the organization's courses the caller may read. Responses are delivered as Server-Sent Events (SSE).",
    responses={
        200: {
...
async def api_rag_chat(
    ...
    - If course_uuid is omitted, searches courses the caller may read in the org.
    """
    course_id = None
    org_id = None
    ...
    if course_id is None:
        ...
        allowed_course_ids = []
        for candidate in org_courses:
            try:
                await check_resource_access(
                    request,
                    db_session,
                    current_user,
                    candidate.course_uuid,
                    AccessAction.READ,
                )
                ...
                if exc.status_code in (403, 404):
                    continue
                raise
            allowed_course_ids.append(candidate.id)
