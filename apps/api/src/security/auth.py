...
#### Classes ####################################################
async def security_get_user(
    request: Request,
    db_session: AsyncSession,
    email: str,
) -> User | None:
    """Resolve an authenticated principal without loading signup services."""
...
async def get_current_user(
    ...
    if username:
        ...
        if trellis_subject is not None:
            from src.services.auth.trellis_sso import introspect_trellis_subject

            if not await introspect_trellis_subject(str(trellis_subject)):
