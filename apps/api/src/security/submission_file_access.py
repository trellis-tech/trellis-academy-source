...
def _file_owned_by_user(filename: str, user_id: int) -> bool:
    """Verify the immutable owner marker added by the upload service."""
    return f"_submission_user_{user_id}." in filename
...
async def enforce_submission_file_access(
    ...
    # The learner-controlled submission JSON cannot establish ownership by
    # itself. Require the immutable user marker inserted by the upload service,
    # then also require the learner's submission to reference that exact name.
    if not _file_owned_by_user(filename, current_user.id):
        raise HTTPException(status_code=403, detail="Access denied")
