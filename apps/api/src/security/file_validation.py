...
# This API returns accepted uploads as one ``bytes`` object. Until callers use
# streaming storage, cap every accepted body to a bounded per-request footprint
# regardless of the broader product/storage limit configured for its type.
_MAX_IN_MEMORY_UPLOAD = 64 * 1024 * 1024
...
FILE_TYPES = {
    ...
    'video': {
        ...
        'max_size': _MAX_IN_MEMORY_UPLOAD,
...
def validate_upload(
    ...
    configured_limit = max_size or config.get('max_size')
    size_limit = (
        min(configured_limit, _MAX_IN_MEMORY_UPLOAD)
        if configured_limit
        else _MAX_IN_MEMORY_UPLOAD
    )
