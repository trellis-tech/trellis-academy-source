...
class CSRFProtectionMiddleware(BaseHTTPMiddleware):
    ...
    async def _is_verified_custom_domain_origin(
        self,
        ...
        # A verified domain is trusted only for mutations addressed to that
        # same domain. Global verification alone would let one tenant's origin
        # submit cookie-authenticated requests to a sibling tenant domain.
        destination = destination_host.split(":", maxsplit=1)[0].lower().rstrip(".")
        if host.rstrip(".") != destination:
            return False
            ...
            async with _async_session_factory() as session:
                row = (await session.execute(
                    select(CustomDomain).where(
                        ...
                        CustomDomain.status.in_(("verified", "active")),
                    )
                )).scalars().first()
    ...
    def _is_csrf_exempt(self, request: Request) -> bool:
        ...
        # Resend delivery feedback is a service-to-service Svix webhook. Keep
        # the exemption route-specific; the handler verifies all three Svix
        # headers against LEARNHOUSE_RESEND_WEBHOOK_SECRET before processing.
        if (
            request.url.path.endswith("/internal/emails/delivery-events")
            and request.headers.get("svix-id")
            and request.headers.get("svix-timestamp")
            and request.headers.get("svix-signature")
        ):
            return True
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if not self.is_allowed_origin(origin, referer):
            ...
            if not (
                candidate
                and await self._is_verified_custom_domain_origin(
                    candidate,
