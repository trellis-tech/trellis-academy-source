...
class ResourceAccessChecker:
    ...
    def __init__(
        ...
        self._linked_usergroup_cache: dict[str, bool] = {}
    ...
    async def check_access(
        ...
        if isinstance(self.current_user, SuperadminAPITokenUser):
            return AccessDecision(
                allowed=False,
                reason="Superadmin API tokens cannot access learner resources",
                resource_uuid=resource_uuid,
                action=action.value,
                context=context.value,
            )
        ...
        # API tokens must use the permission bucket of the requested resource,
        # including child resources such as activities. The token checker
        # resolves the parent chain only to enforce the organization boundary.
        if isinstance(self.current_user, APITokenUser):
            return await self._check_api_token_access(
                resource_uuid,
                action,
                config,
                target_org_id,
        ...
        # For child resources, delegate human-user access to the parent resource
        ...
        if config.parent_resource_type:
            ...
            if (
                ...
                and not decision.via_admin
                and not decision.via_authorship
            ):
                lock_denial = await self._child_lock_denial(resource_uuid, config)
                if lock_denial is not None:
                    return lock_denial
            # A child can carry a stricter UserGroup lock than its parent.
            # Parent authors/admins may still manage it, but a public/role grant
            # must not disclose a locked chapter or activity.
            if (
                action == AccessAction.READ
                and decision.allowed
                and not decision.via_admin
                and not decision.via_authorship
                and await self._has_linked_usergroups(resource_uuid)
            ):
                if not await self._check_usergroup_membership(resource_uuid):
                    return AccessDecision(
                        allowed=False,
                        reason="User is not a member of the linked UserGroup",
                        resource_uuid=resource_uuid,
        ...
        if (
            ...
            and resource_uuid.endswith("_x")
            and target_org_id is None
        ):
            return AccessDecision(
                allowed=False,
                reason="Top-level creation requires an explicit target organization",
                resource_uuid=resource_uuid,
                user_id=self._get_user_id(),
                action=action.value,
                context=context.value,
        ...
        await self._enforce_org_mfa_policy(resource_uuid, config, target_org_id)
    ...
    async def _child_lock_denial(
        self,
        resource_uuid: str,
        config: ResourceConfig,
    ) -> Optional[AccessDecision]:
        """Return a denial when a retained course child has a stricter lock."""
        if config.resource_type not in ("coursechapters", "activities"):
            return None
        ...
        if resource is None:
            return AccessDecision(
                allowed=False,
                reason="Course content not found",
                resource_uuid=resource_uuid,
                user_id=self._get_user_id(),
                action="read",
            )

        denial = await self._lock_policy_denial(
            resource_uuid, getattr(resource, "lock_type", "public")
        )
        if denial is not None:
            return denial

        if config.resource_type == "activities":
            for chapter in await self._activity_chapters(resource.id):
                denial = await self._lock_policy_denial(
                    chapter.chapter_uuid, chapter.lock_type
                )
                if denial is not None:
                    denial.resource_uuid = resource_uuid
                    return denial
        return None

    async def _lock_policy_denial(
        self,
        resource_uuid: str,
        lock_type: object,
    ) -> Optional[AccessDecision]:
        lock_value = getattr(lock_type, "value", lock_type)
        if lock_value == "public":
            return None

        user_id = self._get_user_id()
        if lock_value == "authenticated":
            if user_id != 0:
                return None
            reason = "Authentication required for this course content"
        elif lock_value == "restricted":
            has_linked_group = await self._has_linked_usergroups(resource_uuid)
            if (
                user_id != 0
                and has_linked_group
                and await self._check_usergroup_membership(resource_uuid)
            ):
                return None
            reason = "UserGroup membership required for this course content"
        else:
            reason = "Course content has an unsupported access policy"
    ...
    async def _activity_chapters(self, activity_id: int) -> list[object]:
        """Load every containing chapter so the strictest lock wins."""
    ...
    async def _check_anonymous_read_access(
        ...
        if config.supports_usergroups and await self._has_linked_usergroups(resource_uuid):
            return AccessDecision(
                allowed=False,
                reason="Resource is restricted to a UserGroup",
                resource_uuid=resource_uuid,
                user_id=0,
                action="read",
            )
    ...
    async def _check_public_view_read_access(
        ...
        if config.has_published_field:
            if is_public and is_published and not has_usergroup_lock:
                return AccessDecision(
                    allowed=True,
                    reason="Resource is public and published",
                    via_public=True,
            ...
            if is_public and not has_usergroup_lock:
                return AccessDecision(
                    allowed=True,
                    reason="Resource is public",
                    via_public=True,
        ...
        if (
            is_public
            and not has_usergroup_lock
            and (not config.has_published_field or is_published)
        ):
    ...
    async def _check_write_access(
        ...
        if action == AccessAction.CREATE and resource_uuid.endswith("_x"):
            return await self._check_create_permission(
                resource_uuid, config, target_org_id
            )
    ...
    async def _check_api_token_access(
        ...
        if action == AccessAction.CREATE and resource_uuid.endswith("_x"):
            if target_org_id is None or target_org_id != api_token_user.org_id:
                return AccessDecision(
                    allowed=False,
                    reason=(
                        "API token creation target organization is missing or "
            ...
            if isinstance(rights, dict):
                ...
                has_permission = bool(resource_rights.get("action_create", False))
            else:
                resource_rights = getattr(rights, config.resource_type, None)
                has_permission = bool(
                    getattr(resource_rights, "action_create", False)
                    if resource_rights
                    else False
                )
            ...
            return AccessDecision(
                ...
                reason=(
                    "API token has create permission"
                    if has_permission
                    else f"API token cannot create {config.resource_type}"
                ),
                resource_uuid=resource_uuid,
                action=action.value,
            )
        ...
        parent_uuid = resource_uuid
        parent_config = config
        while resource_org_id is None and parent_config.parent_resource_type:
            parent_uuid = await self._resolve_parent_resource_uuid(parent_uuid, parent_config)
            if not parent_uuid:
                return AccessDecision(
                    allowed=False,
                    reason="Could not resolve the resource organization",
                    resource_uuid=resource_uuid,
                    action=action.value,
                )
            parent_config = RESOURCE_CONFIGS[parent_config.parent_resource_type]
            parent = await self._get_resource(parent_uuid, parent_config)
            if not parent:
                return AccessDecision(
                    allowed=False,
                    reason="Parent resource not found",
                    resource_uuid=resource_uuid,
                    action=action.value,
                )
            resource_org_id = getattr(parent, "org_id", None)

        if resource_org_id != api_token_user.org_id:
            return AccessDecision(
                allowed=False,
    ...
    async def _enforce_org_mfa_policy(
        self,
        resource_uuid: str,
        config: ResourceConfig,
        target_org_id: Optional[int] = None,
    ...
    async def _has_linked_usergroups(self, resource_uuid: str) -> bool:
        cached = self._linked_usergroup_cache.get(resource_uuid)
        if cached is not None:
            return cached
        statement = select(UserGroupResource).where(
            UserGroupResource.resource_uuid == resource_uuid
        ).limit(1)
        linked = (await self.db_session.execute(statement)).scalars().first() is not None
        self._linked_usergroup_cache[resource_uuid] = linked
        return linked
...
def _get_request_checker(
    ...
    existing = getattr(request_state, "rbac_checker", None)
    if (
    ...
    try:
        if request_state is not None:
            request_state.rbac_checker = checker
    except Exception:
