...
def require_create_access(
    ...
    async def dependency(
        ...
        resource_uuid = f"{config.uuid_prefix}x"

        raw_org_id = request.path_params.get("org_id")
        if raw_org_id is None:
            raw_org_id = request.query_params.get("org_id")
        try:
            target_org_id = int(raw_org_id)
        except (TypeError, ValueError):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Creation requires an explicit organization ID",
            )
        if target_org_id <= 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Creation requires a valid organization ID",
            )
