...
function devPort(name: string, fallback: string): string {
...
const DEV_DB_PORT = devPort('LEARNHOUSE_DEV_DB_PORT', '5432')
const DEV_REDIS_PORT = devPort('LEARNHOUSE_DEV_REDIS_PORT', '6379')
const DEV_HOST = process.env.LEARNHOUSE_DEV_HOST

if ((process.env.LEARNHOUSE_DEV_DB_PORT || process.env.LEARNHOUSE_DEV_REDIS_PORT) && !DEV_HOST) {
  throw new Error('LEARNHOUSE_DEV_HOST is required when overriding dev infrastructure ports')
}
if (DEV_HOST && !/^[a-zA-Z0-9.-]+$/.test(DEV_HOST)) {
  throw new Error('LEARNHOUSE_DEV_HOST must be a hostname or IP address')
}
...
export async function devCommand(opts: { ee?: boolean; adminEmail?: string; adminPassword?: string }) {
  ...
  if (!root) {
    p.log.error('Not inside the Trellis Academy source tree.')
    p.log.info('Run this command from the Academy root containing apps/api/ and apps/web/.')
    process.exit(1)
  }

  p.intro(pc.cyan('Trellis Academy Dev Mode'))
  ...
  serviceEnv = {
    ...
    ...(DEV_HOST && {
      LEARNHOUSE_SQL_CONNECTION_STRING: `postgresql://learnhouse:learnhouse@${DEV_HOST}:${DEV_DB_PORT}/learnhouse`,
      LEARNHOUSE_REDIS_CONNECTION_STRING: `redis://${DEV_HOST}:${DEV_REDIS_PORT}/learnhouse`,
    }),
  ...
  console.log()
  console.log(pc.dim('  Trellis Academy services are ready.'))
  console.log()
  ...
  const shutdown = async () => {
    ...
    console.log(pc.dim('Trellis Academy services stopped.'))
    process.exit(0)
  }
