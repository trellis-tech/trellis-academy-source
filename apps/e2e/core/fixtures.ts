...
import { LocalOutboundWriteReceipt, isLoopbackUrl } from './runtime'
...
interface AcademyFixtures {
  outboundWriteReceipt: LocalOutboundWriteReceipt
}

export const test = base.extend<AcademyFixtures>({
  outboundWriteReceipt: [
    async ({ context }, use) => {
      const receipt = new LocalOutboundWriteReceipt()
      await context.route('**/*', async (route) => {
        const request = route.request()
        receipt.observe(request.method(), request.url())
        const isWrite = ['POST', 'PUT', 'PATCH', 'DELETE'].includes(request.method())
        if (isWrite && !isLoopbackUrl(request.url())) {
